...
type Vehicle = { id: string; identifier: string; vehicle_type: string; availability: Availability; location: Location | Location[] | null };
type Incident = { id: string; request_id: string; title: string; location: string | null; latitude: number | null; longitude: number | null; incident_status: string };
...
export default function ResourcesPage() {
  ...
  const load = useCallback(async () => {
    const [locationResult, vehicleResult, incidentResult, teamResult, inventoryResult] = await Promise.all([
      ...
      db.from("vehicles").select("id,identifier,vehicle_type,availability,location:operational_locations(id,name,location_type,latitude,longitude,availability,contact)").order("identifier"),
      db.from("deals").select("id,request_id,title,location,latitude,longitude,incident_status").neq("incident_status", "resolved").order("created_at", { ascending: false }),
  ...
  const nearby = useMemo(() => {
    ...
    const ambulanceCandidates = vehicles
      .filter((vehicle) => vehicle.vehicle_type.toLowerCase().includes("ambulance"))
      .flatMap((vehicle) => {
        const location = Array.isArray(vehicle.location) ? vehicle.location[0] : vehicle.location;
        return location?.latitude != null && location.longitude != null ? [{ ...vehicle, latitude: Number(location.latitude), longitude: Number(location.longitude) }] : [];
      });
    const centerCandidates = locations.filter((location) => location.location_type === "relief_center" && location.latitude != null && location.longitude != null).map((location) => ({ ...location, latitude: Number(location.latitude), longitude: Number(location.longitude) }));
    const center = nearestAvailable(origin, centerCandidates);
    const centerInventory = center ? inventory.filter((item) => item.location_id === center.id && item.availability === "available") : [];
    return { ambulance: nearestAvailable(origin, ambulanceCandidates), center, centerInventory };
  }, [selectedIncident, vehicles, locations, inventory]);
  ...
  async function createTeam() {
    if (!accountId || !teamName.trim()) return;
    const user = (await db.auth.getSession()).data.session?.user;
    if (!user) return;
    await db.from("response_teams").insert({ account_id: accountId, user_id: user.id, name: teamName.trim(), location_id: teamLocationId || null, availability });
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-border bg-card p-4"><h2 className="font-semibold">Find nearby available resources</h2><p className="mt-1 text-xs text-muted-foreground">Recommendation only: distance uses stored coordinates and availability is coordinator-maintained.</p><select value={selectedIncidentId} onChange={(event) => setSelectedIncidentId(event.target.value)} className="mt-3 h-9 w-full rounded-lg border border-border bg-muted px-2 text-sm"><option value="">Select an active incident with coordinates</option>{incidents.map((incident) => <option key={incident.id} value={incident.id}>{incident.request_id} — {incident.title}</option>)}</select>{selectedIncident && (selectedIncident.latitude == null || selectedIncident.longitude == null) && <p className="mt-3 text-sm text-amber-600">Add incident coordinates before distance ranking is available.</p>}{nearby && <div className="mt-4 grid gap-3 sm:grid-cols-2"><Result label="Nearest suitable ambulance" value={nearby.ambulance ? `${nearby.ambulance.identifier}: ${nearby.ambulance.distanceKm.toFixed(1)} km` : "No verified available ambulance with coordinates"} /><Result label="Nearest suitable relief center" value={nearby.center ? `${nearby.center.name}: ${nearby.center.distanceKm.toFixed(1)} km` : "No verified available relief center with coordinates"} detail={nearby.centerInventory.length ? nearby.centerInventory.map((item) => `${item.item_name}: ${item.quantity} ${item.unit}`).join(" · ") : "No available inventory recorded at this centre"} /></div>}</section>
