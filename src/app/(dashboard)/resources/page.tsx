...
type Inventory = { id: string; item_category: string; item_name: string; quantity: number; unit: string; availability: Availability };
...
export default function ResourcesPage() {
  ...
  const load = useCallback(async () => {
    const [locationResult, vehicleResult, incidentResult, teamResult, inventoryResult] = await Promise.all([
      ...
      db.from("relief_inventory").select("id,item_category,item_name,quantity,unit,availability").order("item_name"),
    ]);
  ...
  const nearby = useMemo(() => {
    ...
    return { ambulance: nearestAvailable(origin, ambulanceCandidates), center: nearestAvailable(origin, centerCandidates) };
...
function Result({ label, value }: { label: string; value: string }) { return <div className="rounded-lg bg-muted p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-medium text-foreground">{value}</p></div>; }
