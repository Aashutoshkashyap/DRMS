 ...
 *
 * Auth: Vercel Cron uses `CRON_SECRET`; legacy external schedulers can
 * continue using `AUTOMATION_CRON_SECRET`. The two endpoints
 * (`/api/automations/cron` and this one) are independent operations;
 * we keep them on separate URLs so one failing doesn't block the other.
 *
